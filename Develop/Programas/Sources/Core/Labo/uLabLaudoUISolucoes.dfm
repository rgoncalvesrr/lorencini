inherited LabLaudoUISolucoes: TLabLaudoUISolucoes
  Caption = 'Solu'#231#245'es'
  ClientHeight = 544
  ClientWidth = 1179
  KeyPreview = False
  ExplicitWidth = 1197
  ExplicitHeight = 591
  PixelsPerInch = 120
  TextHeight = 20
  inherited sGrid: TJvStringGrid
    Top = 109
    Width = 1173
    Height = 375
    OnDblClick = sGridDblClick
    OnKeyDown = sGridKeyDown
    OnKeyPress = sGridKeyPress
    ExplicitLeft = 3
    ExplicitTop = 109
    ExplicitWidth = 1173
    ExplicitHeight = 375
  end
  object GroupBox1: TGroupBox [1]
    AlignWithMargins = True
    Left = 3
    Top = 3
    Width = 1173
    Height = 100
    Align = alTop
    Caption = ' Informa'#231#245'es do Ensaio '
    TabOrder = 1
    object Panel1: TPanel
      AlignWithMargins = True
      Left = 5
      Top = 25
      Width = 1163
      Height = 70
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 0
      object Panel2: TPanel
        Left = 0
        Top = 0
        Width = 108
        Height = 70
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 0
        object Label1: TLabel
          AlignWithMargins = True
          Left = 3
          Top = 3
          Width = 102
          Height = 20
          Align = alTop
          Caption = 'Ensaio'
          ExplicitLeft = 4
          ExplicitTop = 4
          ExplicitWidth = 43
        end
        object edEnsaio: TEdit
          AlignWithMargins = True
          Left = 3
          Top = 29
          Width = 102
          Height = 38
          TabStop = False
          Align = alClient
          ReadOnly = True
          TabOrder = 0
          ExplicitHeight = 28
        end
      end
      object Panel3: TPanel
        Left = 1003
        Top = 0
        Width = 160
        Height = 70
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 1
        object Label3: TLabel
          AlignWithMargins = True
          Left = 3
          Top = 3
          Width = 154
          Height = 20
          Align = alTop
          Caption = 'Valor'
          ExplicitLeft = 4
          ExplicitTop = 4
          ExplicitWidth = 34
        end
        object edValor: TJvEdit
          AlignWithMargins = True
          Left = 3
          Top = 29
          Width = 154
          Height = 38
          TabStop = False
          Align = alClient
          Alignment = taRightJustify
          ReadOnly = True
          TabOrder = 0
          ExplicitHeight = 28
        end
      end
      object Panel4: TPanel
        Left = 108
        Top = 0
        Width = 895
        Height = 70
        Align = alClient
        BevelOuter = bvNone
        TabOrder = 2
        object Label2: TLabel
          AlignWithMargins = True
          Left = 3
          Top = 3
          Width = 889
          Height = 20
          Align = alTop
          Caption = 'Nome'
          ExplicitLeft = 4
          ExplicitTop = 4
          ExplicitWidth = 41
        end
        object edNome: TEdit
          AlignWithMargins = True
          Left = 3
          Top = 29
          Width = 889
          Height = 38
          TabStop = False
          Align = alClient
          ReadOnly = True
          TabOrder = 0
          ExplicitHeight = 28
        end
      end
    end
  end
  object PageControl1: TPageControl [2]
    AlignWithMargins = True
    Left = 3
    Top = 109
    Width = 1173
    Height = 375
    Align = alClient
    TabOrder = 2
    TabStop = False
    OnChange = PageControl1Change
  end
  object Panel6: TPanel [3]
    AlignWithMargins = True
    Left = 3
    Top = 490
    Width = 1173
    Height = 51
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 3
    object BitBtn1: TBitBtn
      AlignWithMargins = True
      Left = 1023
      Top = 3
      Width = 147
      Height = 45
      Action = actOk
      Align = alRight
      Caption = 'Confirmar'
      TabOrder = 0
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00A4B6A40017731700277C2700B4BEB400}
    end
  end
  object JvCalcEdit1: TJvCalcEdit [4]
    Left = 437
    Top = 195
    Width = 162
    Height = 28
    ButtonWidth = 28
    TabOrder = 4
    Visible = False
    DecimalPlacesAlwaysShown = False
    OnExit = JvCalcEdit1Exit
    OnKeyPress = JvCalcEdit1KeyPress
  end
  inherited alDef: TActionList
    inherited actCancel: TAction
      ShortCut = 0
    end
  end
  object qSolucoes: TZQuery
    Connection = DM.Design
    SQL.Strings = (
      
        'select s.recno, s.codigo, s.lotefor, s.validade, s.qtd, s.saldo,' +
        ' e.qtd uso,'
      '       f.empresa, f.nome_chave, f.cnpj, (e.recno is null) criar '
      '  from labsol s'
      '       left join labamostras_res_sol e'
      '         on e.relato_recno = :relato'
      '        and e.ensaio_recno = :ensaio'
      '        and e.laudo = :laudo'
      '        and e.solucao = s.recno'
      '       left join fornecedores f'
      '         on f.codigo = s.codigo'
      ' where s.tipo = :tipo'
      ' order by s.validade, s.recno')
    Params = <
      item
        DataType = ftUnknown
        Name = 'relato'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'ensaio'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'laudo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'tipo'
        ParamType = ptUnknown
      end>
    Left = 168
    Top = 64
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'relato'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'ensaio'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'laudo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'tipo'
        ParamType = ptUnknown
      end>
    object qSolucoesrecno: TIntegerField
      FieldName = 'recno'
      Required = True
    end
    object qSolucoeslotefor: TStringField
      FieldName = 'lotefor'
      Size = 40
    end
    object qSolucoesvalidade: TDateField
      FieldName = 'validade'
      DisplayFormat = 'dd/mm/yyyy'
    end
    object qSolucoesqtd: TFloatField
      FieldName = 'qtd'
      Required = True
      DisplayFormat = ',0.###0'
    end
    object qSolucoesuso: TFloatField
      FieldName = 'uso'
      Required = True
      DisplayFormat = ',0.###0'
    end
    object qSolucoessaldo: TFloatField
      FieldName = 'saldo'
      Required = True
      DisplayFormat = ',0.###0'
    end
    object qSolucoescodigo: TIntegerField
      FieldName = 'codigo'
    end
    object qSolucoesnome_chave: TStringField
      FieldName = 'nome_chave'
      Size = 60
    end
    object qSolucoescnpj: TStringField
      FieldName = 'cnpj'
      EditMask = '99.999.999/9999-99;0;'
      Size = 14
    end
    object qSolucoesempresa: TStringField
      FieldName = 'empresa'
      Required = True
      Size = 60
    end
    object qSolucoescriar: TBooleanField
      FieldName = 'criar'
      ReadOnly = True
    end
  end
  object qTipo: TZQuery
    Connection = DM.Design
    SQL.Strings = (
      'select t.recno, t.descri'
      '  from labens_soltipo e'
      '       join labsoltipo t'
      '         on t.recno = e.labsoltipo_recno'
      ' where e.labens_recno = :ensaio')
    Params = <
      item
        DataType = ftUnknown
        Name = 'ensaio'
        ParamType = ptUnknown
      end>
    Left = 104
    Top = 64
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ensaio'
        ParamType = ptUnknown
      end>
    object qTiporecno: TIntegerField
      FieldName = 'recno'
      Required = True
    end
    object qTipodescri: TStringField
      FieldName = 'descri'
      Required = True
      Size = 60
    end
  end
end
